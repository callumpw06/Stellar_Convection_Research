...
if BC_TYPE == 'no-slip':
    logger.info("Applying NO-SLIP boundaries to adjoint solver.")
    problem.add_equation("lambda_adj(z=0) = 0")
    problem.add_equation("lambda_adj(z=1) = 0")
elif BC_TYPE == 'free-slip':
    logger.info("Applying FREE-SLIP boundaries to adjoint solver.")
    problem.add_equation("lambda_adj(z=0)@ez = 0")
    problem.add_equation("ex@(ez@grad_lambda)(z=0) = 0")
    problem.add_equation("lambda_adj(z=1)@ez = 0")
    problem.add_equation("ex@(ez@grad_lambda)(z=1) = 0")
else:
    raise ValueError(f"Unknown boundary condition: {BC_TYPE}")
