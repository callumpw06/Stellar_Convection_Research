"""
This script will plot the heat flux across the top layer as a function of time for a 2D Rayleigh-Benard 
...
for i, file_path in enumerate(files):
    with h5py.File(file_path, 'r') as f:
        chunk_data = f[f'tasks/heat_flux_top'][:,0]  # Load heat flux data
...
# 5. Plot the results
plt.plot(full_times, full_data, label='Heat Flux at Top Layer')
plt.xlabel('Time')
plt.ylabel('Heat Flux at Top Layer')
plt.title('Heat Flux vs Time')
plt.grid(True)
plt.savefig('Heat_flux_vs_time.png', dpi=300, bbox_inches='tight')
plt.show()
...
print(f"Time-averaged Heat Flux at Top Layer: {nu_avg:.4f}")